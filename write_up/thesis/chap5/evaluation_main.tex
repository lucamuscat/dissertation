...
framework is used to characterize the performance of each queue, and will be the
basis of any inference or comparison made in this study. Each section in this
chapter will be dedicated to the performance of each queue\textemdash under a workload of
a fixed number of threads, and varying delay between queue operations. A
queue's performance is quantified using the net runtime of each benchmark (as
described in chapter \ref{}), together with the magnitude of performance
degradation, which will be calculated as the ratio of net runtime between the
current and previous thread (where a value greater than one indicates a
...
Under the \emph{pairwise benchmark}, the \emph{MS-Queue} and the \emph{Baskets
Queue} using tagged pointers are at most \textbf{3.168} and \textbf{2.593}
times faster than their DWCAS counterparts; The \emph{Baskets Queue} is
consistently outperformed by the \emph{MS-Queue} and the \emph{Two-Lock Queue}
(respectively, at most \textbf{0.450} and \textbf{0.516} times slower).
Although the \emph{pairwise benchmark} does not show which queue is
consistently faster, the \emph{50\% enqueue benchmark} shows that the
\emph{Two-Lock Queue} consistently outperforms the \emph{MS-Queue} (by at most
\textbf{0.27} times).
...
Queues using tagged pointers tend to experience higher contention, consequently
leading to a harsher degradation in performance; 
...
At delays greater than 200 nanoseconds, the \emph{two-lock queue} exhibits the best
performance; non-blocking queues using tagged pointers remain competitive in
...
delay), however, as contention drops, the two-lock queue tends to become
...
databases, concurrent queueing algorithms do not have a standardized
benchmarking methodology, allowing researchers to possibly game their results
by choosing a benchmark that favour their algorithm. 
