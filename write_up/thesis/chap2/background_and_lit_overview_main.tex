...
\emph{response}~\citep[Section~3.6.1]{herlihy2020art}. A method \emph{takes
effect} when it succeeds to modify a concurrent object. A history is said to be
\emph{legal} with respect to a correctness property if it is \emph{equivalent}
to a history that respects said correctness property.
...
\citeauthor{herlihy1990linearizability} define \emph{linearizability} as a
correctness condition that provides the illusion that each operation takes
effect instantaneously at some point between a method's invocation and response~\cite{herlihy1990linearizability}.
