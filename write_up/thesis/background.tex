...
Over the years, as the exponential increase in Uniprocessor performance
...
\begin{definition}[Multiprocessor]
    Multiprocessors consist of multiple hardware processors, each of which
    executes a sequential program\cite[Appendix~B.2]{herlihy2020art}.
\end{definition}

\begin{definition}[Time Slice]
    A \emph{`Time Slice'} is a small unit of time that a thread may consume
    before being paused, and swapped to another thread\cite{osconcepts2021}.
\end{definition}
...
use a \emph{context switch} in order to swap the thread with another. A thread
may be \emph{descheduled} voluntarily before its time slice expires by yielding
control to the CPU, or when an interrupt or a system call occurs
\cite[Section~3.2.3]{osconcepts2021}
\end{definition}
...
size~\cite[Section~3.2.1]{perfbook2021}. Cache lines are filled with data from
the requested memory address, together with neighbouring bytes.

\emph{Sharing} occurs when overlapping data is loaded into several cache lines.
Following the MESI protocol, cache lines become \emph{invalidated} when
modified in a shared state, leading to the invalidation of cache lines
containing shared data. 

In some cases, sharing is unavoidable; it is possible for closely located,
yet unrelated data to share a single cache line.
\emph{False Sharing} occurs when the invalidation of a cache line invalidated
