    ...
    \begin{algorithmic}[1]
        ...
        \Repeat
            \While{atomic\_load\_explicit(lock.busy, memory\_order\_acquire)}
                ...
                \For{$i \rightarrow 0$}
                    \State PAUSE \Comment{Reduces contention from spin-waiting}
                \EndFor
                \If{$\text{mask} < \text{max\_backoff}$}
        ...
        \Until{\text{atomic\_exchange\_explicit(lock.busy, 1,
        memory\_order\_acquire)}}
        \State \Return \Comment{Enter Critical Section}
    \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        \State \text{atomic\_store\_explicit(lock.busy, 0, memory\_order\_release)}
    \end{algorithmic}
\end{algorithm}
