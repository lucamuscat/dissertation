...
% TODO: Add machine info and clock resolution.
...
biases). Randomness aids with exercising a larger portion of code paths.
...
The 50\% enqueue benchmark will execute at most
half the number of enqueues and dequeues as the pairwise
enqueue-dequeue benchmark, as an iteration no longer consists of a pair of
enqueues and dequeues, but just one of them.
...
\emph{random errors}~\citep{lelbach2015cppbenchmarking}. Systemic errors are caused by the inherent inaccuracies
...
(granularity) of the clock used to measure elapsed time~\citep{lelbach2015cppbenchmarking}. The CPU's clock speed is
